...
const PRESETS: Record<
  string,
  { prompt: string; agents: Record<string, AgentDefinition> }
> = {
  'code-review-team': {
    prompt:
      'Review the sandbox directory comprehensively: check code quality, security issues, and test coverage. Use the code-reviewer, security-scanner, and test-runner agents.',
    agents: {
      'code-reviewer': {
        description: 'Expert code reviewer for quality and style improvements.',
        prompt:
          'You are an expert code reviewer. Analyze code quality, style, and maintainability. Check for code smells, poor patterns, and opportunities for improvement.',
        tools: ['Read', 'Glob', 'Grep'],
      },
      'security-scanner': {
        description: 'Security expert who finds vulnerabilities.',
        prompt:
          'You are a security expert. Scan code for vulnerabilities, unsafe patterns, injection risks, and security anti-patterns.',
        tools: ['Read', 'Grep'],
      },
      'test-runner': {
        description: 'Test specialist who checks test coverage.',
        prompt:
          'You are a testing expert. Check for test coverage, test quality, and missing test cases.',
        tools: ['Bash', 'Read'],
      },
    },
  },
  'research-team': {
    prompt:
      'Research the Claude Agent SDK: search for documentation, summarize key features, and explain how to use it effectively.',
    agents: {
      'web-researcher': {
        description: 'Web researcher who finds and collects information.',
        prompt:
          'You are a web researcher. Search the web for information and fetch relevant pages.',
        tools: ['WebSearch', 'WebFetch'],
      },
      summarizer: {
        description: 'Expert at distilling and summarizing information.',
        prompt:
          'You are a summarization expert. Take information and distill it into clear, concise summaries.',
        tools: ['Read'],
      },
    },
  },
...
subagentsRouter.post('/query', (req: Request, res: Response) => {
  ...
  sendEvent(res, 'subagent_config', {
    agents: agentNames,
    prompt: finalPrompt,
  });

  console.log(agents, 'agents', finalPrompt, 'finalPrompt');
