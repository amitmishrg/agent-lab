...
sessionsRouter.get('/', async (_req: Request, res: Response) => {
  try {
    const sdkSessions = await listSessions({ limit: 100, dir: SANDBOX_DIR });
    const dbSessions = db
...
sessionsRouter.post('/:id/resume', (req: Request, res: Response) => {
  ...
  const cwd = wrongCwd ? '/tmp/wrong-cwd-demo' : (session?.cwd ?? SANDBOX_DIR);
